## jaeger

Jaeger backend v2

### Synopsis

Jaeger backend v2

```
jaeger [flags]
```

### Options

```
      --config --config=file:/path/to/first --config=file:path/to/second   Locations to the config file(s), note that only a single location can be set per flag entry e.g. --config=file:/path/to/first --config=file:path/to/second. (default [])
      --feature-gates flag                                                 Comma-delimited list of feature gate identifiers. Prefix with '-' to disable the feature. '+' or no prefix will enable the feature. (default -connector.spanmetrics.legacyMetricNames,-exporter.UsePullingBasedExporterQueueBatcher,-filter.filterlog.useOTTLBridge,-filter.filtermetric.useOTTLBridge,-filter.filterspan.useOTTLBridge,jaeger.sampling.includeDefaultOpStrategies,pkg.translator.prometheus.NormalizeName,-pkg.translator.prometheus.PermissiveLabelSanitization,-processor.tailsamplingprocessor.metricstatcountspanssampled,receiver.jaeger.DisableRemoteSampling,-service.noopTracerProvider,-service.profilesSupport,telemetry.UseLocalHostAsDefaultMetricsAddress,-telemetry.disableAddressFieldForInternalTelemetry,-telemetry.disableHighCardinalityMetrics,telemetry.useOtelWithSDKConfigurationForInternalTelemetry)
  -h, --help                                                               help for jaeger
      --set func                                                           Set arbitrary component config property. The component has to be defined in the config file and the flag has a higher precedence. Array config properties are overridden and maps are joined. Example --set=processors.batch.timeout=2s
```

### SEE ALSO

* [jaeger completion](jaeger_completion.md)	 - Generate the autocompletion script for the specified shell
* [jaeger components](jaeger_components.md)	 - Outputs available components in this collector distribution
* [jaeger docs](jaeger_docs.md)	 - Generates documentation
* [jaeger validate](jaeger_validate.md)	 - Validates the config without running the collector
* [jaeger version](jaeger_version.md)	 - Print the version.

###### Auto generated by spf13/cobra on 16-Feb-2026
